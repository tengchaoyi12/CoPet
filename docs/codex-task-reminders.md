...
- 宠物始终显示静态帧，不随任务状态播放动画或音效。
- 可以拖拽宠物调整位置；短按宠物会打开 Codex 主界面，拖拽不会误触发打开。
- 任务需要权限或用户输入时，显示“任务需要你处理，快去看看吧。”。
- 任务完成时，显示“任务完成啦，快去看看吧。”。
...
- 未处理提醒在 CoPet 重启后恢复。
- 在 macOS 上，当 `com.openai.codex` 成为前台应用时，CoPet 自动清除全部完成提醒，但保留等待和失败提醒。其他平台暂不支持前台自动清理。
...
2. 确认宠物保持静态；拖拽可以调整位置且不会打开 Codex。
3. 短按宠物，确认打开 Codex 主界面。
4. 在 Codex 提交一个简单任务，等任务结束后确认出现“任务完成啦，快去看看吧。”。
5. 点击气泡，确认打开对应 Codex 任务。
6. 同时准备完成、等待和失败提醒，切换到其他应用后再让 Codex 成为前台；确认全部完成提醒消失，等待和失败提醒保留。
7. 关闭或点击提醒后重启 CoPet，确认已处理提醒不恢复。
8. 保留一条未处理等待或失败提醒后重启，确认提醒恢复。
9. 开启“登录后自动启动”，重新进入设置确认开关仍反映系统真实状态。
